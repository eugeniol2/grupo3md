...
filtro_opcao = st.selectbox(
    "Selecione um aspecto para analisar a evasão:",
    [
        "Região",
        "UF",
        "Cidade",
...
if filtro_opcao == "Região":
    ...
    filtro_valor = st.slider(
        "Selecione uma faixa etária:", min_value=0, max_value=100, value=(0, 100)
    )
    dados_filtrados = censo[
        (
            censo["QT_CONC_18_24"]
            + censo["QT_CONC_25_29"]
            + censo["QT_CONC_30_34"]
            ...
            + censo["QT_CONC_40_49"]
            + censo["QT_CONC_50_59"]
            + censo["QT_CONC_60_MAIS"]
        ).between(*filtro_valor)
    ]
    filtro_titulo = (
        f"Evasão dos Cursos para Faixa Etária {filtro_valor[0]}-{filtro_valor[1]}"
    )
    ...
    ranking_estado = (
        dados_estado.groupby(dados_estado["NO_CURSO"].str.lower())["QT_CONC"]
        .sum()
    ...
    ranking_nordeste = (
        dados_nordeste.groupby(dados_nordeste["NO_CURSO"].str.lower())["QT_CONC"]
        .sum()
        .sort_values(ascending=False)
        .head(5)
    )
...
if dados_filtrados is not None:
    ...
    ax.bar(
        ["Evasão", "Permanência"],
        [dados_filtrados["QT_CONC"].sum(), dados_filtrados["QT_MAT"].sum()],
    )
