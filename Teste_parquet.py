...
censo2020 = pd.read_parquet("data/censo2020_filtrado.parquet")
censo2021 = pd.read_parquet("data/censo2021_filtrado.parquet")
...
st.subheader(
    "O calculo é feito em todos as linhas de cursos encontradas de acordo com os filtros selecionados, é então feita uma média para uma melhor observação das taxas. Uma taxa de Z-score também foi aplicada para evitar dados inconsistentes"
)
...
if len(selected_course_data) == 0:
    ...
    abandonment_rates = selected_course_data.apply(
        lambda row: calcular_taxa_evasao(row, selected_course_data), axis=1
    )
    ...
    if len(valid_abandonment_rates) == 0:
        ...
        z_score_threshold = 2
        valid_abandonment_rates_no_outliers = valid_abandonment_rates[
        ...
        st.subheader(
            f"Média de evasão (sem outliers): {mean_abandonment_rate_no_outliers:.2f}%"
        )
